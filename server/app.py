...
with gr.Blocks(title="Eco-LLM Routing") as demo:
    ...
    with gr.Row():
        task_selector = gr.Dropdown(["task_1", "task_2", "task_3"], value="task_1", label="Task")
        reset_btn = gr.Button("🔄 Reset", variant="primary")
    
    obs_box = gr.Textbox(label="State", lines=6, interactive=False)
    
    with gr.Row():
        strategy_dd = gr.Dropdown(["NONE", "USE_CACHE", "DO_CASCADE", "EARLY_EXIT", "WAIT", "CALL_KB"], value="NONE", label="Strategy")
        model_dd = gr.Dropdown(["SMALL", "MEDIUM", "LARGE"], value="SMALL", label="Model")
